        ...
        try {
            ...
            if ($response->successful()) {
                ...
                if (isset($data['data']['conditions'][0])) {
                    ...
                    // Конвертація температури з Фаренгейта в Цельсій
                    $tempF = $conditions['temp'] ?? 0;
                    $tempC = round(($tempF - 32) * 5 / 9);
                    
                    // Конвертація швидкості вітру з mph в м/с
                    // Використовуємо середню швидкість за 10 хвилин
                    $windMph = $conditions['wind_speed_avg_last_10_min'] ?? $conditions['wind_speed_last'] ?? 0;
                    $windMs = round($windMph * 0.44704, 1);
                    
                    // Вологість
                    $humidity = round($conditions['hum'] ?? 0);
